...
export function EditModal({ book }) {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      toast.update(toastId, {
        render: "Failed to update book. Please try again.",
        type: "error",
        isLoading: false,
        autoClose: 3000
      });
        ...
        <Modal.Backdrop>
          <Modal.Container placement="bottom-center" className="sm:baseline-center">
            <Modal.Dialog className="w-full max-w-full sm:max-w-md bg-white rounded-t-3xl sm:rounded-3xl overflow-hidden shadow-2xl border border-gray-100 max-h-[90vh] flex flex-col">
              <Modal.CloseTrigger className="cursor-pointer top-4 right-4 absolute p-1 text-gray-400 hover:text-gray-600" onClick={() => setIsOpen(false)} />
                ...
                <Surface variant="default">
                  <form id="edit-book-form" onSubmit={onSubmit} className="flex flex-col gap-4 sm:gap-5">
                    ...
                    <div className="w-full">
                      ...
                      <Input name="title" type="text" defaultValue={book?.title || ""} placeholder="e.g., The Great Gatsby" className="w-full" required />
                    </div>

                    <div className="w-full">
                      <label className="text-xs sm:text-sm font-semibold text-gray-700 mb-1 block">Description</label>
                      <Input name="description" type="text" defaultValue={book?.description || ""} placeholder="Write a brief summary..." className="w-full" required />
                    </div>

                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div>
                        <label className="text-xs sm:text-sm font-semibold text-gray-700 mb-1 block">Price ($)</label>
                        <Input name="price" type="number" step="0.01" defaultValue={book?.price || ""} placeholder="e.g., 12.99" className="w-full" required />
                      </div>
                      <div>
                        <label className="text-xs sm:text-sm font-semibold text-gray-700 mb-1 block">Quantity</label>
                        <Input name="quantity" type="number" defaultValue={book?.quantity || ""} placeholder="e.g., 10" className="w-full" required />
                      </div>
                    </div>

                    <div className="w-full">
                      <label className="text-xs sm:text-sm font-semibold text-gray-700 mb-1 block">Book Cover Image</label>
                      <input name="image" type="file" accept="image/*" className="w-full text-xs sm:text-sm text-slate-500 file:mr-3 file:py-2 file:px-3 file:rounded-xl file:border-0 file:font-semibold file:bg-purple-50 file:text-purple-700 hover:file:bg-purple-100 cursor-pointer" />
                    </div>

                    <Modal.Footer className="pt-4 border-t border-gray-50 flex flex-col-reverse sm:flex-row justify-end gap-2 mt-4 shrink-0 px-0">
                      <Button onPress={() => setIsOpen(false)} variant="flat" className="w-full sm:w-auto cursor-pointer bg-gray-100 text-gray-600 rounded-xl font-medium">Cancel</Button>
                      <Button type="submit" form="edit-book-form" disabled={loading} className="w-full sm:w-auto bg-purple-600 text-white hover:bg-purple-700 font-semibold sm:px-6 rounded-xl shadow-lg disabled:opacity-50 cursor-pointer">
